  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const [index, good] of goods.entries()) {
      const data = {
        ...
        pricePerSqm: good.availableInExpress && good.pricingMode === 'area' ? good.pricePerSqm ?? 0 : null,
        fixedPrice: good.availableInExpress && good.pricingMode === 'fixed' ? good.fixedPrice ?? 0 : null,
        individualPricePerSqm: good.availableInIndividual && good.pricingMode === 'area' ? good.individualPricePerSqm ?? 0 : null,
        individualFixedPrice: good.availableInIndividual && good.pricingMode === 'fixed' ? good.individualFixedPrice ?? 0 : null,
        availableInExpress: good.availableInExpress,
        availableInIndividual: good.availableInIndividual,
